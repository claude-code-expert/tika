...
Phase 1에서 MVP 칸반 보드(체크리스트, 라벨, 상위 이슈, 담당자, 인증, 워크스페이스 포함)를 완성하고, Phase 2에서 알림 및 고급 기능 기반 SaaS로 확장한다.
...
| Phase | 버전 | 테마 | 상태 |
|-------|------|------|------|
| Phase 1 | v0.1.0 | MVP — 칸반 보드 + 체크리스트 + 라벨 + 상위 이슈 + 담당자 + 인증 + 워크스페이스 | 구현 완료 |
| Phase 2 | v0.2.0 | SaaS — 알림 + 라벨 확장 + 댓글 + 고급검색 | 구현 예정 |
...
| 포함 | 비고 |
|------|------|
| Google OAuth 인증 | NextAuth.js v5 기반 세션 관리 |
| 기본 워크스페이스 | 첫 로그인 시 자동 생성, 데이터 격리 |
...
| 담당자 배정 | 로그인 시 자동 멤버 생성, 본인만 배정 가능 |
...
#### FR-011: 담당자 배정
- Phase 1: 로그인 시 자동 멤버 생성 (수동 등록 불가)
- Phase 1: 담당자는 본인만 배정 가능 (자동 입력, 읽기 전용)
- 미배정(null) 가능, 카드에 아바타(이니셜 + 배경색) 표시
- 보드 필터링 지원
- 상세 사양: REQUIREMENTS.md FR-011 참조

#### FR-012: 워크스페이스
- 첫 로그인 시 기본 워크스페이스("내 워크스페이스") 자동 생성
- 모든 데이터(tickets, labels, issues, members)를 workspace_id로 격리
- Phase 1: 사용자당 1개 워크스페이스, 다른 사용자 데이터 접근 불가
- 상세 사양: REQUIREMENTS.md FR-012 참조

#### FR-013: Google OAuth 인증
- Google OAuth 2.0 기반 로그인/로그아웃 (NextAuth.js v5)
- 최초 로그인 시 자동: 사용자 생성 → 워크스페이스 생성 → 멤버 등록
- 모든 API에 세션 검증, 미인증 요청은 401 또는 로그인 페이지 리다이렉트
- 상세 사양: REQUIREMENTS.md FR-013 참조

### Phase 2 기능 (FR-102 ~ FR-107)
...
#### FR-105: 라벨 확장 (워크스페이스별 격리)
Phase 1에서 workspace_id FK로 격리 구현 완료. Phase 2에서 설정 페이지 UI 통합.
...
### 시나리오 8: Google 계정 로그인 (Phase 1)

1. 비로그인 상태로 서비스 접근 시 로그인 페이지로 이동
2. Google 로그인 버튼 클릭 → OAuth 인증
3. 최초 로그인 시 기본 워크스페이스("내 워크스페이스")와 멤버가 자동 생성
4. 내 칸반 보드가 표시되고, 헤더에 프로필 아바타와 이름 표시
5. 로그아웃 시 로그인 페이지로 이동
...
| NFR-007 | 인증 보안 | NextAuth.js JWT, httpOnly 쿠키, CSRF 방지 |

### Phase 2

| ID | 항목 | 기준 |
|----|------|------|
| NFR-101 | SaaS 운영 | 커스텀 도메인, 99.5% 가용성 |
| NFR-102 | 인증 보안 | Phase 1 NFR-007에서 구현 완료. Phase 2에서 추가 보안 강화 |
...
| 인증 | NextAuth.js v5 | Google OAuth 인증, 세션 관리 |
