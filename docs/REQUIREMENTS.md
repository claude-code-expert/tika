...
| Phase   | 버전   | 테마                                                     | 상태      | 레포 |
| ------- | ------ | -------------------------------------------------------- | --------- | ---- |
| Phase 1 | v0.1.0 | MVP — 칸반 보드 + 체크리스트 + 라벨 + 상위 이슈 + 담당자 + 인증 + 워크스페이스 | 구현 완료 | tika |
| Phase 2 | v0.2.0 | SaaS — 알림 + 라벨 확장 + 댓글 + 고급검색                                     | 구현 예정 | tika |
...
| type        | enum            | X    | GOAL, STORY, FEATURE, TASK 중 택 1  | TASK   |
...
| name         | VARCHAR(20) | NOT NULL                   | —         | 라벨명         |
| color        | VARCHAR(7)  | NOT NULL                   | '#3B82F6' | HEX 색상       |
| created_at   | TIMESTAMPTZ | NOT NULL                   | now()     | 생성 시각      |

**제약**: `UNIQUE(workspace_id, name)` — 워크스페이스 내 라벨명 중복 불가
...
- 로그인 시 워크스페이스에 멤버 자동 생성 (수동 등록 아님)
- Phase 1: 자기 자신만 담당자로 배정 가능 (다른 멤버 선택 불가)
- 티켓당 1명의 담당자 배정 또는 미배정(null)
...
- `POST /api/members` — 멤버 등록 (201) → **Phase 4에서 활성화** (Phase 1: 자동 생성만)
- `PATCH /api/members/:id` — 멤버 수정 (200) → **Phase 4에서 활성화**
- `DELETE /api/members/:id` — 멤버 삭제 (204) → **Phase 4에서 활성화**

> Phase 1에서 POST/PATCH/DELETE 요청 시 405 Method Not Allowed 반환

**UI 요소**:

- 티켓 생성/수정 모달: 담당자 표시 (Phase 1: 읽기 전용, 자기 자신 자동 입력) + 미배정 버튼
...
**설명**: 데이터 격리 단위로 워크스페이스를 사용한다. 사용자의 모든 데이터(티켓, 라벨, 이슈, 멤버)는 워크스페이스에 소속된다.
...
| -------- | --------- | -------------------------- |
| id       | number    | PK                         |
| name     | string    | 워크스페이스 이름 (1~100자) |
| ownerId  | string    | FK→users(id), 소유자       |
| createdAt | timestamp | 생성 시각                  |

**처리 규칙**:

- 첫 로그인 시 기본 워크스페이스("내 워크스페이스") 자동 생성
- tickets, labels, issues, members 테이블에 `workspace_id` FK 추가
- 모든 데이터 조회/생성 시 현재 워크스페이스로 자동 스코핑 (`WHERE workspace_id = ?`)
- Phase 1: 사용자당 1개 워크스페이스 (멀티 워크스페이스는 Phase 4)
...
**tickets 테이블 변경**: `workspace_id INT NOT NULL FK→workspaces(id)` 칼럼 추가
...
- 최초 로그인 시 기본 워크스페이스 + 멤버 자동 생성
- 세션 기반 인증 (NextAuth.js v5)
- 모든 API 요청에 세션 검증 적용
- 인증되지 않은 요청은 로그인 페이지로 리다이렉트

**첫 로그인 플로우**: Google OAuth → user 생성 → 기본 워크스페이스 생성 → member 자동 생성 → 보드 리다이렉트

**DB 테이블**: `users`

| 칼럼       | 타입         | 제약             | 기본값 | 설명                            |
| ---------- | ------------ | ---------------- | ------ | ------------------------------- |
| id         | TEXT         | PK               | —      | 고유 식별자 (OAuth provider ID) |
...
**API**: NextAuth.js v5 자동 라우트 (`/api/auth/*`)
...
| FR-009 | 라벨/태그   | 색상 라벨로 카테고리 분류 (워크스페이스별 격리)           | `/api/labels`                |
| FR-010 | 상위 이슈   | Goal→Story→Feature 계층 연결                             | `/api/issues`                |
| FR-011 | 담당자      | 멤버 배정 (Phase 1: self-only)                           | `/api/members`               |
| FR-012 | 워크스페이스 | 데이터 격리 단위, 자동 생성                              | `/api/workspaces`            |
| FR-013 | Google OAuth | 인증, 세션 관리, 자동 사용자 등록                        | `/api/auth/*`                |
...
- [ ] 로그인 시 워크스페이스에 멤버가 자동 등록된다
- [ ] 티켓 생성/수정 시 담당자가 자기 자신으로 자동 입력된다 (Phase 1)
- [ ] "미배정" 상태로 둘 수 있다
...
- [ ] 최초 로그인 시 기본 워크스페이스와 멤버가 자동 생성된다
...
| US-013: Google 로그인                | FR-012, FR-013 | —                                    |
...
{
  "error": {
    "code": "UNAUTHORIZED | VALIDATION_ERROR | TICKET_NOT_FOUND | INTERNAL_ERROR",
...
| 에러 코드        | HTTP 상태 | 발생 조건                          |
| ---------------- | --------- | ---------------------------------- |
| UNAUTHORIZED     | 401       | 미인증 요청 (세션 없음/만료)       |
...
> 알림, 라벨 확장(워크스페이스별 격리 강화), 댓글, 고급 검색을 추가한다.
> 인증 및 워크스페이스는 Phase 1(FR-012, FR-013)에서 구현 완료.
...
### FR-105: 라벨 확장 — 워크스페이스별 라벨 관리

**설명**: Phase 1에서 구현된 라벨 기능(FR-009)의 워크스페이스별 격리를 강화한다. 멀티 워크스페이스 환경에서 각 워크스페이스가 독립적인 라벨을 관리할 수 있도록 한다.

> 기본 라벨 CRUD, 티켓 부착, 필터링은 Phase 1 FR-009에서 구현 완료.
> workspace_id FK 및 `UNIQUE(workspace_id, name)` 제약은 Phase 1 FR-009에서 적용 완료.

**확장 내용**:

- 멀티 워크스페이스 간 라벨 복사/이동 기능
- 워크스페이스당 최대 20개 라벨 제한 강화
...
| FR-105 | 라벨 확장       | 워크스페이스별 라벨 관리 (FR-009 확장)  |
...
| 데이터 격리   | 워크스페이스별 데이터 완전 분리 (workspace_id FK) |
...
> US-101(내 계정으로 로그인)은 Phase 1 US-013으로 이동됨.
...
**관련 FR**: FR-009 (Phase 1, workspace_id 격리 포함), FR-105 (Phase 2 확장)
