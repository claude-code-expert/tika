...
**프로젝트 타입**: 웹 (app/는 Next.js 라우트, src/server/는 백엔드, src/client/는 프런트엔드, src/shared/는 공통 코드용 모노레포)
...
├── client/                    # 프런트엔드 레이어 (이 기능에서 미사용)
│   ├── components/
│   ├── hooks/
│   └── api/
└── shared/                    # 프런트엔드와 백엔드 공유
    ├── types/
...
**구조 결정**: 웹 애플리케이션 패턴 (템플릿의 옵션 2). Next.js App Router는 단일 모노레포에서 프런트엔드와 백엔드를 모두 제공. `app/api/`의 API 라우트, `src/server/`의 백엔드 로직, `src/client/`의 프런트엔드 로직, `src/shared/`의 공유 코드. 이 구조는 관련 코드를 함께 유지하면서 관심사 분리를 강제.
